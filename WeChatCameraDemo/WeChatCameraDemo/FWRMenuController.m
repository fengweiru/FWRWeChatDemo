...
@implementation FWRMenuController
...
- (void)viewDidLoad {
    ...
    self.buttonView.frame = CGRectMake(0, MAINSCREEN.size.height-height, MAINSCREEN.size.width, height);
...
- (int) renderButtons
{
    ...
    for (int i = 0; i < self.namesArrays.count+1; i++) {
        ...
        button.tag = i;
        button.frame = CGRectMake(0, height, MAINSCREEN.size.width, 50);
        ...
        if (i < self.namesArrays.count-1) {
            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, height, MAINSCREEN.size.width, 1)];
            lineView.backgroundColor = [UIColor clearColor];
            [self.buttonView addSubview:lineView];
            height += 1;
        }else if (i == self.namesArrays.count-1) {
            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, height, MAINSCREEN.size.width, 5)];
            lineView.backgroundColor = [UIColor clearColor];
            [self.buttonView addSubview:lineView];
            height += 5;
        }
